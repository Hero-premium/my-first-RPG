...
public class Dialogs {
	...
	String[] updateStory(Entity player) {
		return new String[] {
				...
				"gatekeeper: but " + player.name
						+ ", that is such a dangerous mission, how are you planning on doing it?",
				player.name
						+ ": of course I didn't come unprepared, I do have some moves that will help me defeat this evil monster",
				"gatekeeper: and what are they...?",
				player.name
						+ ": dash! an awesome ability that would help me dodge the dragon attacks, sometimes I don't move fast enough though",
	...
	void checkInput(Label dialogLabel, TextField takeInput, Player player, CombatEntity enemy, Stage stage) {
		...
		if (story[index].equals(
				"gatekeeper: oh you shouldn't have told me that, as I'll be your first foe, I'll defend my lord with my life!")) {
			BattleLauncher.launchBattle(player, enemy,stage);
